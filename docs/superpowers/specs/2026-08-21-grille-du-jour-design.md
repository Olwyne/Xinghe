...
| 7 | Débordement | **Tronquer à la frontière**, avec un liseré sur le bord concerné. Cohérent avec la règle « une session appartient à la période de son début ». |
...
- **Bords tronqués** : liseré sur le côté concerné quand `clippedStart` ou `clippedEnd`.
...
`calendar.day`, `calendar.today`, `calendar.previousDay`, `calendar.nextDay`, `calendar.noTask`, `calendar.attachToTask`, `calendar.emptyDay`, `calendar.continuesBefore`, `calendar.continuesAfter`, `calendar.attachFailed`.
...
- **Troncature** : session commencée avant la fenêtre, terminée après, à cheval sur chaque bord ; drapeaux `clippedStart`/`clippedEnd` ; positions bornées à `[0, 1]`.
